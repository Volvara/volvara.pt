(function(){
  ...
  async function loadData() {
    try {
      ...
      else if(cached) { localStorage.removeItem('superstars_cache'); } // clear invalid cache
  ...
  function renderPublicBracket(torneio) {
    ...
    function pubFinal(j, medal, placeLabel) {
      ...
      var champCell = '<div class="pub-champion-cell">' +
        ...
        '</div>' +
      '</div>';
      return '<div class="pub-final-wrap">' + matchHtml + (done ? champCell : '') + '</div>';
    }
    ...
    if(qp && qp.rondas && qp.rondas.length) {
      ...
      html+=pubCol(t('final12'),[pubFinal(fJogo,'🥇','1st')]);
      html+='</div></div></div>';
    }
    ...
    if(qb && qb.rondas && qb.rondas.length) {
      ...
      html+=pubCol(t('finalQB'),[pubFinal(finalB,null,getLang()==='en'?'11th':'11º')]);
      html+='</div></div></div>';
    }

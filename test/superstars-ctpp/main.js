(function(){
  ...
  var LIVE_READ_PAT = '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'; // Set new PAT via Claude Code
  ...
  var CACHE_TTL  = 60 * 1000;  // 60s — refresh quickly with live polling
  ...
  async function ghFetchJSON(filename) {
    if(!LIVE_READ_PAT) {
      // Fallback: raw.githubusercontent.com (may have CDN cache)
      var rawBase = API_BASE
        .replace('https://api.github.com/repos/','https://raw.githubusercontent.com/')
        .replace('/contents/','/main/');
      var res = await fetch(rawBase + filename + '?t=' + Date.now());
      if(!res.ok) throw new Error('raw fetch ' + res.status);
      return await res.json();
    }
    // Authenticated Contents API — no CDN cache
    var url = API_BASE + filename;
    var headers = {
      'Authorization': 'token ' + LIVE_READ_PAT,
      'Accept': 'application/vnd.github.v3+json',
    };
    if(_etags[filename]) headers['If-None-Match'] = _etags[filename];
    var res = await fetch(url, {headers: headers, cache: 'no-store'});
    if(res.status === 304) return null; // unchanged
    if(res.status === 401 || res.status === 403) {
      // PAT invalid/expired — clear it and fallback to raw
      console.warn('[SuperStars] PAT auth failed (' + res.status + '), falling back to raw');
      LIVE_READ_PAT = '';
      var rawBase = API_BASE
        .replace('https://api.github.com/repos/','https://raw.githubusercontent.com/')
        .replace('/contents/','/main/');
      var r2 = await fetch(rawBase + filename + '?t=' + Date.now());
      if(!r2.ok) throw new Error('raw fallback ' + r2.status);
      return await r2.json();
    }
    if(!res.ok) throw new Error('gh api ' + res.status);
    var etag = res.headers.get('ETag');
    if(etag) _etags[filename] = etag;
    var body = await res.json();
    // Contents API returns base64-encoded content — ghDecode handles decode+parse
    return ghDecode(body);
  }
  ...
  async function ghPollChanged(filename) {
    if(!LIVE_READ_PAT) {
      // Without PAT: fetch full file, compare hash
      var rawBase = API_BASE
        .replace('https://api.github.com/repos/','https://raw.githubusercontent.com/')
        .replace('/contents/','/main/');
      var res = await fetch(rawBase + filename + '?t=' + Date.now());
    ...
    // With PAT: use ETag — if 304, file unchanged
    var url = API_BASE + filename;
    var headers = {
      'Authorization': 'token ' + LIVE_READ_PAT,
      'Accept': 'application/vnd.github.v3+json',
    ...
    if(res.status === 401 || res.status === 403) {
      // PAT invalid/expired — clear and signal change so raw fallback runs
      console.warn('[SuperStars] PAT auth failed, clearing');
      LIVE_READ_PAT = '';
      return 'REFETCH'; // signal livePoll to re-fetch via ghFetchJSON (which will use raw)
    }
    if(!res.ok) return false;
    var etag = res.headers.get('ETag');
    if(etag) _etags[filename] = etag;
    var body = await res.json();
    return JSON.stringify(ghDecode(body)); // return as string for livePoll to parse
  }
  ...
  async function loadData() {
    try {
      ...
      if(cached) { applyData(cached); renderProxima(); renderRankingSection(rankEtapaActual); renderCalendario(); }

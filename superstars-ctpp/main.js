(function(){
  ...
  async function loadData() {
    try {
      ...
      var noCache = { headers: {'Cache-Control':'no-cache','Pragma':'no-cache'} };
      var ts = '?t='+Date.now();
      var [jogRes, etaRes, torRes] = await Promise.all([
        fetch(rawBase+'jogadores.json'+ts, noCache),
        fetch(rawBase+'etapas.json'+ts, noCache),
        fetch(rawBase+'torneios.json'+ts, noCache),
      ]);
...
// ══════════════════════════════════════════════════════════════════════
  ...
  function renderResEtapaContent(etapaId) {
    ...
    if(!gruposComJogadores.length) {
      html += '<div class="res-empty">Sorteio ainda não realizado.</div>';
    }
  ...
  async function livePoll() {
    try {
      ...
      var noCache = {headers: {'Cache-Control':'no-cache', 'Pragma':'no-cache'}};
      var torRes = await fetch(rawBase+'torneios.json'+ts, noCache);
      if(!torRes.ok) return;

...
// ══════════════════════════════════════════════════════════════════════
  ...
  function renderResultadosSection() {
    ...
    wrapEl.innerHTML = '<div class="res-pills">'+pills+'</div>';
  ...
  function calcGrupoClassif(g) {
    ...
    (g.jogadores||[]).forEach(function(id) {
      stats[id] = {jogId:id, pts:0, vitorias:0, derrotas:0, jogos:0, sg:0};
    });
    (g.jogos||[]).forEach(function(j) {
      if(!stats[j.ja]||!stats[j.jb]) return;
      stats[j.ja].jogos++; stats[j.jb].jogos++;
      stats[j.ja].sg += (j.score_a||0)-(j.score_b||0);
      stats[j.jb].sg += (j.score_b||0)-(j.score_a||0);
  ...
  function renderPublicBracket(torneio) {
    ...
    function pubMatch(j) {
      ...
      var isByeA = !j.jogA||(j.bye&&!j.jogA), isByeB = !j.jogB||(j.bye&&!j.jogB);
      function playerHtml(name, win, isBye, score) {
        return '<div class="pub-player'+(win?' pub-winner':'')+(isBye?' pub-bye':'')+'">'+
          '<span class="pub-pname">'+name+'</span>'+
          (score!==null&&score!==undefined&&done?'<span class="pub-score'+(win?' pub-score-w':'')+'">'+score+'</span>':'')+'</div>';
      }
      return '<div class="pub-match">'+
        playerHtml(nA,winA,isByeA,j.score_a)+
        playerHtml(nB,winB,isByeB,j.score_b)+
      '</div>';
    }
    ...
    function losersBlock(title, jogo) {
      if(!jogo) return '';
      return '<div class="pub-losers-block">'+
        '<div class="pub-losers-title">'+title+'</div>'+
        pubMatch(jogo)+
      '</div>';
    }

    function placeBlock(title, jogId) {
      if(!jogId) return '';
      return '<div class="pub-losers-block">'+
        '<div class="pub-losers-title">'+title+'</div>'+
        '<div class="pub-match"><div class="pub-player pub-winner"><span class="pub-pname">'+pNome(jogId)+'</span></div></div>'+
      '</div>';
    }
    ...
    if(qp && qp.rondas && qp.rondas.length) {
      ...
      var r2  = qp.rondas.find(function(r){return r.id==='r2';});
      var r3  = qp.rondas.find(function(r){return r.id==='r3';});
      var r4  = qp.rondas.find(function(r){return r.id==='r4';});
      var fJogo = r4 ? r4.jogos.find(function(j){return j.id==='qp_final';}) : null;

      html += '<div class="pub-bracket-wrap">';
      html += '<div class="pub-bracket-label">Quadro Principal</div>';
      html += '<div class="pub-bracket-scroll"><div class="pub-bracket-inner">';
      if(r1) html += col('Oitavos', r1.jogos.map(pubMatch));
      if(r2) html += col('Quartos', r2.jogos.map(pubMatch));
      if(r3) html += col('Meias', r3.jogos.map(pubMatch));
      html += col('Final', [pubMatch(fJogo)]);
      html += '</div></div></div>';
    }
    ...
    if(qb && qb.rondas && qb.rondas.length) {
      ...
      var r2qb = qb.rondas.find(function(r){return r.id==='r2';});
      var r3qb = qb.rondas.find(function(r){return r.id==='r3';});
      var finalB = r3qb ? r3qb.jogos.find(function(j){return j.id==='qb_final';}) : null;

      html += '<div class="pub-bracket-wrap">';
      html += '<div class="pub-bracket-label">Quadro B (11º – 15º)</div>';
      html += '<div class="pub-bracket-scroll"><div class="pub-bracket-inner">';
      if(r1qb) html += col('Quartos', r1qb.jogos.map(pubMatch));
      if(r2qb) html += col('Meias', r2qb.jogos.map(pubMatch));
      html += col('Final QB', [pubMatch(finalB)]);
      html += '</div></div></div>';
    }

    // ── Losers / placement matches ─────────────────────────────────────
    if(qp && qp.rondas && qp.rondas.length) {
      var r3p2 = qp.rondas.find(function(r){return r.id==='r3p';});
      var r4b  = qp.rondas.find(function(r){return r.id==='r4';});
      var r3qb2= qb && qb.rondas ? qb.rondas.find(function(r){return r.id==='r3';}) : null;
      var r1qb2= qb && qb.rondas ? qb.rondas.find(function(r){return r.id==='r1';}) : null;

      var t34   = r4b  ? r4b.jogos.find(function(j){return j.id==='qp_3lugar';}) : null;
      var sfp1  = r3p2 ? r3p2.jogos[0] : null;
      var sfp2  = r3p2 ? r3p2.jogos[1] : null;
      var t56   = r4b  ? r4b.jogos.find(function(j){return j.id==='qp_56';}) : null;
      var t78   = r4b  ? r4b.jogos.find(function(j){return j.id==='qp_78';}) : null;
      var t910  = r4b  ? r4b.jogos.find(function(j){return j.id==='qp_910';}) : null;
      var t1314 = r3qb2? r3qb2.jogos.find(function(j){return j.id==='qb_1314';}) : null;

      html += '<div class="pub-losers-wrap">';
      html += '<div class="pub-bracket-label">Posições por Lugar</div>';
      html += '<div class="pub-losers-grid">';
      html += losersBlock('3º / 4º Lugar', t34);
      html += losersBlock('Meia 5/6', sfp1);
      html += losersBlock('5º / 6º Lugar', t56);
      html += losersBlock('Meia 7/8', sfp2);
      html += losersBlock('7º / 8º Lugar', t78);
      html += losersBlock('9º / 10º Lugar', t910);
      html += losersBlock('13º / 14º Lugar', t1314);

      // 15th+ from real QB QF losers
      if(r1qb2) {
        var posExtra = 15;
        r1qb2.jogos.forEach(function(qfj){
          ...
          if(isReal && qfj.vencedor) {
            ...
            if(perd) { html += placeBlock(posExtra+'º Lugar', perd); posExtra++; }
          }
        });
      }

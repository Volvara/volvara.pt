  ...
  function renderProxima(){
    ...
    if(hc) hc.innerHTML =
      '<div class="hero-card">'+
        ...
        '<div class="hero-card-title">'+proxima.n+(getLang()==='en'?' Round':t('etapa'))+'</div>'+
    ...
    if(pe) pe.innerHTML =
      '<div class="next-etapa fade-in">'+
        ...
        '<div class="ne-info">'+
          ...
          '<div class="ne-detail">'+ t('domingo') +' · '+proxima.hora+' — 11h · '+t('vagasLabel')+' · 5€</div>'+
  ...
  function renderRankingSection(etapaId) {
    ...
    if(etapas.length > 0) {
      ...
      opts += etapas.map(function(e){
        var lbl = e.numero+t('etapa')+' · '+e.data.split('-')[2].replace(/^0/,'')+'/' +e.data.split('-')[1];
    ...
    todas.forEach(function(e){
      ...
      html+='<div class="etapa-card'+(past?" done":"")+'">'+
        ...
        '<div class="etapa-info">'+
          ...
          '<div class="etapa-title">'+e.n+(getLang()==='en'?' Round Super Stars':'ª Etapa Super Stars')+'</div>'+
          '<div class="etapa-details">'+ t('domingo') +' · '+e.hora+' — 11h · '+t('vagasLabel')+' · 5€</div>'+
          badge+
        '</div></div>';
    });
...
// ══════════════════════════════════════════════════════════════════════
  ...
  function renderResultadosSection() {
    ...
    var pills = etapasComT.map(function(e) {
      ...
      return '<button class="res-etapa-pill'+(active?' active':'')+
        ...
        (e.numero||'')+(getLang()==='en'?' Round':t('etapa'))+' · '+dayNum+' '+mes+
      '</button>';
    }).join('');
  ...
  function renderResEtapaContent(etapaId) {
    ...
    gruposComJogadores.forEach(function(g, gi) {
      ...
      html += '<div class="res-grupo-header">Grupo '+(gi_real+1)+'</div>';

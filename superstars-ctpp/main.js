(function(){
  ...
  async function loadData() {
    ...
    // Seed SHA from initial load so first poll knows the baseline
    try {
      var headers2 = { Accept:'application/vnd.github.v3+json' };
      fetch(API_BASE+'torneios.json?t='+Date.now(), {headers:headers2})
        .then(function(r){return r.json();})
        .then(function(d){ _liveSHAs.torneios = d.sha; });
    } catch(e){}
    startLivePolling();
  }
...
// ══════════════════════════════════════════════════════════════════════
  ...
  async function livePoll() {
    try {
      ...
      // Fetch only torneios.json SHA first (lightest check)
      var shaRes = await fetch(API_BASE+'torneios.json'+ts, {headers:headers});
      if(!shaRes.ok) return;
      var shaData = await shaRes.json();
      var newSHA = shaData.sha;

      // If SHA unchanged → no update needed
      if(_liveSHAs.torneios && _liveSHAs.torneios === newSHA) {
        updateLiveTimestamp(false); // update timestamp without re-render
        return;
      }

      // SHA changed → full reload
      _liveSHAs.torneios = newSHA;
      ...
      updateLiveTimestamp(true); // re-render resultados
    } catch(e) {

(function(){
  ...
  var RAW_BASE   = 'https://raw.githubusercontent.com/'+REPO_OWNER+'/'+REPO_NAME+'/main/superstars-ctpp/data/';
  ...
  // Etapas de fallback (caso fetch falhe)
  ...
  // ── FETCH DADOS DO GITHUB ─────────────────────────────────────────────────
  async function loadData() {
    try {
      var [jogRes, etaRes, resRes] = await Promise.all([
        fetch(RAW_BASE+'jogadores.json?t='+Date.now()),
        fetch(RAW_BASE+'etapas.json?t='+Date.now()),
        fetch(RAW_BASE+'resultados.json?t='+Date.now())
      ]);
      if(jogRes.ok && etaRes.ok && resRes.ok) {
        var jog = await jogRes.json();
        var eta = await etaRes.json();
        var res = await resRes.json();

        JOGADORES = jog.jogadores || [];

        // Separar etapas senior/junior com formato compatível
        var todasEtapas = eta.etapas || [];
        ETAPAS_SENIOR = todasEtapas
          .filter(function(e){ return e.tipo==='senior'; })
          .map(function(e){ return mapEtapa(e); });
        ETAPAS_JUNIOR = todasEtapas
          .filter(function(e){ return e.tipo==='junior'; })
          .map(function(e){ return mapEtapa(e); });
    ...
    // Renderizar tudo após dados carregados
    renderProxima();
  ...
  function mapEtapa(e) {
    var dateParts = e.data ? e.data.split('-') : [];
    var meses = {
    ...
    return {
      n: e.numero,
      data: e.data,
      dia: e.tipo==='senior'?'Dom':'Sáb',
      mes: meses[dateParts[1]] || '',
      hora: e.hora || '8h30',
  ...
  function useFallback() {
    ETAPAS_SENIOR = ETAPAS_SENIOR_FB;
    ETAPAS_JUNIOR = ETAPAS_JUNIOR_FB;
  }
  ...
  function countdown(targetDate, hora){
    ...
    var h = parseInt(hm[0])||8, m = parseInt(hm[1])||30;
  ...
  function renderProxima(){
    ...
    var details = isSen
      ? "Domingo · "+proxima.hora+" — 11h · 15 vagas · 5€"
      : "Sábado · "+proxima.hora+" · "+(proxima.escaloes||"");
    ...
    var prox = document.getElementById("proxima-etapa-container");
    if(prox) prox.innerHTML =
  ...
  function renderRanking(cat){
    ...
    rows.forEach(function(r,i){
      ...
      html+='<tr class="rank-row"><td><span class="rank-pos '+pc+'">'+(i+1)+'</span></td>'+
        '<td><div class="rank-name">'+r.nome+'</div></td>'+
        '<td style="color:var(--mu);font-size:13px">'+r.jogos+'</td>'+
        '<td style="color:var(--mu);font-size:13px">'+r.vitorias+'</td>'+
        '<td><span class="rank-pts">'+r.pts+'</span></td></tr>';
    });
    html+='</tbody></table>';
  ...
  // ── INIT ──────────────────────────────────────────────────────────────────
  // Mostrar fallback imediatamente, depois actualizar com dados reais
  useFallback();
  renderProxima();
  renderRanking("senior");
  renderCalendario("todos");

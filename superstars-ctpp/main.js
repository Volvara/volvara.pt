(function(){
  ...
  var JOGADORES     = [];
  var JOGOS         = [];
  var ETAPAS_RAW    = []; // todas as etapas do JSON original
  ...
  // ── CALCRANKING (com stats expandidas) ───────────────────────────────────
  // etapaId = null → todas as etapas; etapaId = 'e001' → só essa etapa
  function calcRanking(cat, etapaId) {
    ...
    jogosFiltrados.forEach(function(g){
      ...
      if(g.vencedor===g.ja){
        s[g.ja].pts+=3; s[g.ja].vitorias++; s[g.jb].pts+=1; s[g.jb].derrotas++;
      } else {
        s[g.jb].pts+=3; s[g.jb].vitorias++; s[g.ja].pts+=1; s[g.ja].derrotas++;
      }
    });
    return Object.values(s)
      .map(function(r){
        r.dif_enc   = r.vitorias - r.derrotas;
        r.dif_games = r.games_g  - r.games_p;
        return r;
      })
      .sort(function(a,b){
        return (b.pts-a.pts) || (b.dif_games-a.dif_games) || (b.dif_enc-a.dif_enc);
      });
  }

  // ── ETAPAS COM JOGOS (para selector) ─────────────────────────────────────
  function etapasComJogos(cat) {
    // Só etapas da categoria certa que têm pelo menos 1 jogo
  ...
  function mvpEtapa(cat, etapaId) {
    ...
    return rows[0]; // primeiro após ordenação
  }

  // ── RENDER RANKING SECTION (selector + mvp + tabela) ─────────────────────
  function renderRankingSection(cat, etapaId) {
    ...
    if(!temRes) {
      ...
      // Colunas: Pos(fixo) | Jogador(fixo) | Pts | V | D | Dif.Enc | J | Dif.G
      var thead = '<thead><tr>'+
      ...
      var tbody = rows.map(function(r,i){
        ...
        return '<tr class="rank-row">'+
          ...
          '<td class="col-stat stat-mu">'+r.enc+'</td>'+
    ...
    container.innerHTML = selectorHtml + mvpHtml + tabelaHtml;

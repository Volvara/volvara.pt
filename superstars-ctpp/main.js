(function(){
  ...
  window.openPlayerModal = function(playerId) {
    ...
    var totalStats = calcRanking(cat,'todas').find(function(r){return r.id===playerId;})||{pts:0,vitorias:0,derrotas:0,dif_games:0};
    ...
    var etapasHtml = Object.keys(etapasJogadas).map(function(eid){
      ...
      var jogosHtml = s.jogos.map(function(g){
        ...
        return '<div class="modal-game">'+
          ...
          '<span class="modal-game-score '+(won?'won':'lost')+'">'+scoreA+'–'+scoreB+'</span>'+
          '<span class="modal-game-result '+(won?'win-badge':'loss-badge')+'">'+(won?'V':'D')+'</span>'+
        '</div>';
      }).join('');
      return '<div class="modal-etapa">'+
        '<div class="modal-etapa-hdr"><span class="modal-etapa-lbl">'+lbl+'</span>'+
        '<span class="modal-etapa-pts">'+s.pts+' pts · '+s.v+'V–'+s.d+'D</span></div>'+
        jogosHtml+
      '</div>';
    }).join('');
      ...
      '<div class="modal-box">'+
        ...
        '<div class="modal-header">'+
          ...
          '<div>'+
            ...
            '<div class="modal-cat">'+(cat==='senior'?'Senior · Bola Dura':'Junior · Bola '+cat.charAt(0).toUpperCase()+cat.slice(1))+'</div>'+
          '</div>'+
        '</div>'+

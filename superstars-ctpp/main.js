...
(function(){
  ...
  var JOGADORES = [
    {id:"j001",nome:"João Ferreira",tipo:"senior",escalao:null},
    {id:"j002",nome:"Maria Santos",tipo:"senior",escalao:null},
    {id:"j003",nome:"Pedro Costa",tipo:"senior",escalao:null},
    {id:"j004",nome:"Ana Rodrigues",tipo:"senior",escalao:null},
    {id:"j005",nome:"Carlos Mendes",tipo:"senior",escalao:null},
    {id:"j006",nome:"Sofia Lopes",tipo:"senior",escalao:null},
    {id:"j007",nome:"Rui Almeida",tipo:"senior",escalao:null}
  ];
  var JOGOS = [];
  ...
  function countdown(targetDate){
    var now = new Date();
    var target = new Date(targetDate + "T08:30:00");
    var diff = target - now;
  ...
  function renderProxima(){
    ...
    if(!proxima) return;
    var cd = countdown(proxima.data);
    var isSen = proxima.tipo==="senior";
    var dayNum = proxima.data.split("-")[2].replace(/^0/,"");
    var badgeTxt = isSen?"Senior · Bola Dura":"Junior Stars";
    ...
    document.getElementById("proxima-etapa-container").innerHTML =
      '<div class="next-etapa fade-in">'+
        '<div class="ne-date edb-'+(isSen?"senior":"junior")+'">'+
          '<div class="ne-day">'+dayNum+'</div>'+
          '<div class="ne-month">'+proxima.mes+'</div>'+
        '</div>'+
        '<div class="ne-info">'+
          '<span class="ne-badge'+(isSen?"":" clay")+'">'+badgeTxt+'</span>'+
    ...
    if(cd){
      setInterval(function(){
        var c = countdown(proxima.data); if(!c) return;
  ...
  // RANKING
  function calcRanking(cat){
    var jogs = JOGADORES.filter(function(j){
      if(cat==="senior") return j.tipo==="senior";
      return j.tipo==="junior" && j.escalao===cat;
    });
    var s = {};
    jogs.forEach(function(j){s[j.id]={nome:j.nome,pts:0,jogos:0,vitorias:0};});
    JOGOS.forEach(function(g){
      if(!s[g.ja]||!s[g.jb]) return;
      s[g.ja].jogos++; s[g.jb].jogos++;
      if(g.vencedor===g.ja){s[g.ja].pts+=3;s[g.ja].vitorias++;s[g.jb].pts+=1;}
      else{s[g.jb].pts+=3;s[g.jb].vitorias++;s[g.ja].pts+=1;}
    });
    return Object.values(s).sort(function(a,b){return (b.pts-a.pts)||( b.vitorias-a.vitorias);});
  }

  function renderRanking(cat){
    var rows = calcRanking(cat);
    var container = document.getElementById("ranking-table-container");
    var temRes = rows.some(function(r){return r.pts>0;});
    if(!temRes){
      container.innerHTML='<div class="rank-empty">'+
        '<div style="font-size:2.5rem;margin-bottom:14px">🎾</div>'+
        '<div style="font-size:15px;font-weight:600;color:var(--txt);margin-bottom:6px">Temporada 2026 a começar</div>'+
        '<div>O ranking actualiza após cada etapa. A 1ª etapa '+(cat==="senior"?"Senior realiza-se dia 12 de Abril":"Junior realiza-se dia 18 de Abril")+'.</div></div>';
      return;
    }
    var html='<table class="rank-table"><thead><tr><th style="width:52px">Pos</th><th>Jogador</th><th>J</th><th>V</th><th>Pts</th></tr></thead><tbody>';
    rows.forEach(function(r,i){
      var pc=i===0?"pos-1":i===1?"pos-2":i===2?"pos-3":"pos-n";
      html+='<tr class="rank-row"><td><span class="rank-pos '+pc+'">'+(i+1)+'</span></td><td><div class="rank-name">'+r.nome+'</div></td><td style="color:var(--muted);font-size:13px">'+r.jogos+'</td><td style="color:var(--muted);font-size:13px">'+r.vitorias+'</td><td><span class="rank-pts">'+r.pts+'</span></td></tr>';
    });
    html+='</tbody></table>';
  ...
  window.addEventListener("scroll",function(){
    ...
    for(var i=secs.length-1;i>=0;i--){
      ...
      if(el&&el.offsetTop<=sy){
        ...
        document.querySelectorAll(".nav-link").forEach(function(l){l.classList.toggle("active",l.getAttribute("href")===anchor);});
        break;
      }
    }
  },{passive:true});

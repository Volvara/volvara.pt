...
// ══════════════════════════════════════════════════════════════════════
  ...
  function renderPublicBracket(torneio) {
    ...
    // Helper: render one bracket column
    function bracketCol(ronda, title) {
      if(!ronda||!ronda.jogos) return '';
      var col = '<div class="pub-bracket-col">';
      col += '<div class="pub-ronda-title">'+title+'</div>';
      ronda.jogos.forEach(function(j) {
        var nA = j.jogA ? (JOGADORES.find(function(p){return p.id===j.jogA;})||{}).nome||j.jogA : 'BYE';
        var nB = j.jogB ? (JOGADORES.find(function(p){return p.id===j.jogB;})||{}).nome||j.jogB : 'BYE';
        var done = j.vencedor!=null;
        var winA = done && j.vencedor===j.jogA;
        var winB = done && j.vencedor===j.jogB;
        col += '<div class="pub-match">';
        col += '<div class="pub-player'+(winA?' pub-winner':'')+(j.bye&&!j.jogA?' pub-bye':'')+'">';
        col += '<span class="pub-pname">'+nA+'</span>';
        if(done && j.score_a!==null) col += '<span class="pub-score'+(winA?' pub-score-w':'')+'">'+j.score_a+'</span>';
        col += '</div>';
        col += '<div class="pub-player'+(winB?' pub-winner':'')+(j.bye&&!j.jogB?' pub-bye':'')+'">';
        col += '<span class="pub-pname">'+nB+'</span>';
        if(done && j.score_b!==null) col += '<span class="pub-score'+(winB?' pub-score-w':'')+'">'+j.score_b+'</span>';
        col += '</div>';
        col += '</div>';
      });
      col += '</div>';
    ...
    if(qp && qp.rondas && qp.rondas.length) {
      ...
      html += '<div class="pub-bracket-label">Quadro Principal (1º – 10º)</div>';
      html += '<div class="pub-bracket-scroll"><div class="pub-bracket-inner">';
      var rondaNames = ['Oitavos','Quartos','Meias','3º Lugar','Final'];
      qp.rondas.forEach(function(r, ri) {
        html += bracketCol(r, rondaNames[ri]||r.nome||'R'+(ri+1));
      });
      // Final column (r4)
      var r4 = qp.rondas.find(function(r){return r.id==='r4';});
      if(r4) {
        r4.jogos.forEach(function(j){
          if(j.id==='qp_final'||j.id==='qp_3lugar') {
            // already included in r4 bracketCol
          }
        });
      }
      html += '</div></div></div>';
    ...
    if(qb && qb.rondas && qb.rondas.length) {
      ...
      html += '<div class="pub-bracket-scroll"><div class="pub-bracket-inner">';
      var rondaNamesB = ['Quartos','Meias','13º Lugar','Final QB'];
      qb.rondas.forEach(function(r, ri) {
        html += bracketCol(r, rondaNamesB[ri]||r.nome||'R'+(ri+1));
      });
      html += '</div></div></div>';
    }
